...
async function fetchHtml(url: string, debugBag?: any, retries = 3): Promise<string> {
  ...
  if (isDev) console.log(`[${new Date().toISOString()}] Fetching HTML from ${url}, attempt ${4 - retries}`);
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (isDev) console.error(`[${new Date().toISOString()}] Fetch failed for ${url}: ${e.message}`);
      if (i < retries - 1) {
        if (isDev) console.log(`[${new Date().toISOString()}] Retrying ${url} (${retries - i - 1} attempts left)`);
...
function extractProfileUrlsFromHtml(html: string, debugList?: any): string[] {
  ...
  // Broader selector to catch dynamic links
