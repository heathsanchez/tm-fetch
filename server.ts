...
// Randomize user agents to avoid being blocked
const UA_POOL = [
...
// Launch Playwright browser with Render-safe settings
...
// -----------------------------
// GET /tm/insights (browser-callable version)
// -----------------------------
...
// -----------------------------
// GET /fetch (browser-callable version)
// -----------------------------
app.get("/fetch", async (req, res) => {
  ...
  try {
    ...
    const result = await withBrowser(async ({ browser }) => {
      ...
      const html = await p.content();
      await ctx.close();
      return { url: u.toString(), html, hash: crypto.createHash("md5").update(html).digest("hex") };
    });
